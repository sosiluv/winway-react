    ...
    <div className="min-h-screen bg-white pt-16">
                ...
                {Array.from({ length: Math.ceil(summaryFeatures.length / 4) }).map((_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 transition-colors cursor-pointer ${
                      currentSummarySlide === index ? 'bg-slate-600' : 'bg-slate-300'
                    }`}
                  />
                ))}
              </div>
      ...
      <section className="py-20 bg-gradient-to-r from-slate-50 to-slate-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-slate-900 mb-6 font-cormorant">
            Ready to Transform Your Desktop Trading?
          </h2>
          <p className="text-lg text-slate-600 mb-8 max-w-2xl mx-auto">
            Join leading financial institutions worldwide in revolutionizing their desktop trading operations with our comprehensive platform solutions. Experience unparalleled performance, reliability, and innovation.
          </p>
